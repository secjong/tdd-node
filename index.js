...
function solution() {
  ...
  const imgUrl25 = 'https://pix8.agoda.net/hotelImages/219/21916044/21916044_21010517230093992447.jpg?ca=16&ce=1&s=312x';
  const option = {w: 240};

  // console.log(getResizedImagePath(imgUrl19, option));
  // console.log(getResizedImagePath(imgUrl24, option));
  console.log(getResizedImagePath(imgUrl25));
}
...
function getResizedImagePath(url, size) {
  ...
  if (url.search(/^\/\/\w+(\.\w+)+(\/\w+)+\.\w+$/i) !== -1) {
    // '//openimage.interpark.com/dia/images/288/34/210225034/ecf366fce3af4d1bb8fc45ebb39cd925.jpg'
    result = url;
  }
  /* full url 오는 경우 end */
  /* path만 오는 경우 begin */
  ...
  const queryString = isEmpty(size) ? '' : `?${getQueryStringFromJson(size)}`;

  // 일치하는 패턴이 없는 경우는 잘못된 경로이므로 기본이미지 반환
  return result === null
    ? '//openimage.interpark.com/tour-desktop/common/noimage_277.png'
    : `${result}${queryString}`;
}
